...
void kbase_pm_do_poweron(struct kbase_device *kbdev, bool is_resume, bool hwaccess_locked)
{
		...
		unsigned int flags)
{
	...
	kbase_pm_do_poweron(kbdev, false, false);
	mutex_unlock(&kbdev->pm.lock);
	mutex_unlock(&js_devdata->runpool_mutex);
...
void kbase_hwaccess_pm_resume(struct kbase_device *kbdev)
{
	...
	kbdev->pm.suspending = false;
	kbase_pm_do_poweron(kbdev, true, false);
	/* MALI_SEC_INTEGRATION */
